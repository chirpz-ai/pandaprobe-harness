...
def test_score_record_terminal_and_floats() -> None:
    rec = ScoreRecord.parse(
        {"name": "task_completion", "value": "0.3", "status": "SUCCESS"}
    )
...
def test_score_record_failed_is_terminal_with_null_value() -> None:
    rec = ScoreRecord.parse({"name": "task_completion", "value": None, "status": "FAILED"})
...
def test_run_scores_terminal_logic() -> None:
    payload = {
        "run_id": "r1",
        "scores": [
            {"name": "task_completion", "value": 0.3, "status": "completed"},
            {"name": "coherence", "value": None, "status": "running"},
        ],
    }
    scores = RunScores.parse("r1", payload)
    assert not scores.is_terminal()  # one still running
    assert scores.by_name("task_completion") is not None
