...
from that store as an agent skill::
    ...
    harness_guide.md    skill-style task guide plus compact generated scope
                        index. Contains NO rule text.
    rules/global.md     cross-domain rules available on demand
    rules/scoped.md     default granular guidance
...
chooses a read/search tool. ``scope`` is selected by managed repair using host
scope hints, with ``scoped`` as the normal granular default and ``global``
reserved for genuinely universal guidance.
...
#: Explicit universal scope; never inferred solely from evaluation tier.
GLOBAL_SCOPE = "global"
#: Default granular scope; managed repair may refine it to a custom name.
SCOPED_SCOPE = "scoped"
...
def normalize_scope(value: str | None) -> str:
    ...
    but it is also used as a filename, so it is slugified and bounded here solely
    for path safety. There is no semantic prefix format. Empty or unusable input
    selects the normal ``scoped`` default; ``global`` must be explicit.
    """
...
class Rule:
    ...
    def from_json(cls, data: dict[str, Any]) -> Rule:
        ...
        if raw_scope is None:
            ...
            # global, tagged (i.e. derived from a notice) → the scoped default.
...
class RulesStore:
    """Structured rules + rendered task-facing ``harness_guide.md``."""
    ...
    def _reference_lines(
        ...
        if not groups:
            return [
                "_No learned guidance is available yet._"
            ]
        lines: list[str] = []
        for scope, rules in groups.items():
    ...
    def _scope_intro(scope: str) -> str:
        if scope == GLOBAL_SCOPE:
            return (
                "_Cross-domain guidance. Apply individual rules only when relevant._"
            )
        if scope == SCOPED_SCOPE:
            return (
