...
notices to a workspace that the agent \emph{pulls} on its own initiative, with nothing injected
...
\item \textbf{Self-healing as a drop-in reliability layer.} We deliver self-healing as a runtime
envelope that wraps an \emph{arbitrary} LLM agent loop (model-, framework-, and task-agnostic,
non-blocking, and separated from the agent by a strict \emph{pull-model} boundary that never
mutates the transcript), letting the agent continually improve its own policy-inducing context
across episodes with no weight updates, no human labels, and no human in the loop. To our
knowledge it is the first self-correction loop in which \emph{every} self-authored change must
pass an empirical test before it can influence behavior.
\item \textbf{An evidence gate that makes self-improvement trustworthy.} We formalize rule
validation as a replay-based paired improvement test with a multi-metric non-regression
constraint together with a forward-trial test on breach proportions, backed by a corpus-level
regression guard against catastrophic forgetting (\S\ref{sec:validate}, \S\ref{sec:regression}).
This converts unbounded self-modification into \emph{bounded, auditable, and reversible}
adaptation, and it closes an observability-to-autonomy loop: production session-quality signals
(grounded in trajectory-risk and uncertainty-propagation research) drive the agent's own
corrections through a concrete formalism for the seams (the retrieval operator $\rho$ and the
self-heal operator $\Heal$; \S\ref{sec:harness}).
\item \textbf{A reliability-first evaluation protocol.} We reframe agent progress around
\emph{reliability} ($\passk$: all $k$ trials pass) rather than peak capability ($\passone$), and
give a paired A/B protocol (metric--failure calibration and rule-promotion checkpoints, an
explicit token-overhead confound, and standard paired significance tests) over three agent
benchmarks (\S\ref{sec:setup}).
\end{enumerate}
...
\citep{trivedi2024appworld,merrill2026terminal,barres2025tau}. Paired significance
...
$r$ & a rule (text, tags, metric, rationale) \\
...
\begin{equation}
  ...
  \label{eq:metricvec}
\end{equation}
with higher values better. In our study the evaluator $\evalfn$ is instantiated by
PandaProbe~\citep{Tayebati_pandaprobe_2026}, an open-source agent-engineering and evaluation
platform, and $d{=}2$: $m_1$ is a session \emph{reliability} score and $m_2$ a session
\emph{consistency} score, both in $[0,1]$ (higher is better).
The service is reached through a command-line interface exposing a request/response contract
(arguments in, JSON out); this narrow seam is why the Harness core carries no runtime dependency
on the service and can be exercised against an in-process stand-in. Every call degrades gracefully
to a \emph{pending} (missing) score on failure, so the evaluation path never blocks or raises into
the host agent loop.

\paragraph{Session-quality metrics.} Both metrics aggregate the same per-trace signals (model
confidence, loop detection, tool-call correctness, and coherence) into a single session score,
but with different intent. \emph{Reliability} captures \emph{worst-case} risk: each signal is
converted to a per-trace risk and the session score is a tail-risk aggregate over the most at-risk
traces (a top-$k$ tail blended with the single worst trace), so one catastrophic trace sinks the
score even when the rest are clean. This aggregation follows the trajectory-risk formulation of
TRACER~\citep{tayebati2026tracer}, which aggregates risk over the critical episodes of an agent
trajectory. \emph{Consistency} instead captures \emph{stability}: each trace's confidence-based
uncertainty is amplified by penalties from the other signals, and the per-trace uncertainties are
combined by a root-mean-square across the session, so many moderate deviations score poorly even
when no single trace is catastrophic. This aggregation follows the situational
uncertainty-propagation formulation of SAUP~\citep{zhao2024saup}, which propagates step-wise
uncertainty under situational weighting. The two are complementary: reliability is sensitive to
rare, severe failures and consistency to pervasive instability, and the Harness treats a breach of
\emph{either} as a degradation.

For metric $j$ with threshold $\tau_j$, the \emph{point-breach}
indicator is
\begin{equation}
...
the eval-set}. Its limits are exactly that coverage; cases that cannot be replayed are
...
Throughout, the session-quality metrics that drive the Harness are supplied by the PandaProbe
platform (\S\ref{sec:setup-formal}) through its command-line interface; the same platform,
metrics, and breach thresholds are used across all arms and seeds, so the evaluation signal is
identical in the baseline and Harness runs.
...
\item \textbf{AppWorld}~\citep{trivedi2024appworld}: multi-step tasks over a controllable world of
apps and APIs, scored by task- and scenario-goal completion plus a \emph{no-collateral-damage}
check (the agent must not corrupt unrelated state). We use its development split (57 tasks).
\item \textbf{Terminal-Bench}, driven through the Harbor harness~\citep{merrill2026terminal}:
hard, realistic command-line tasks executed in a sandboxed container, with a per-attempt binary
reward. We use a stratified subset ($\approx\!30$ tasks).
\item \textbf{$\tau^2$-bench}~\citep{barres2025tau}: a dual-control, tool-calling dialogue
benchmark in which an LLM-simulated user and the agent act on a shared customer-service
environment; it reports $\passk$ natively. We use one domain's task set (retail).
\end{itemize}
...
replay wired.
\item \textbf{B$'$, Harness without validation (flag-gated ablation):} identical to B but rules
...
benchmark trials may complete faster than production sessions, the runner explicitly settles
...
model from each of three commercial providers (Google Gemini, OpenAI GPT
and Anthropic Claude); a representative triad (one strong model per provider) is used for the 
headline runs. Trials are drawn from each model's natural nondeterminism: current frontier 
models reject an explicit temperature control, so trial-to-trial variance is \emph{intrinsic} 
and cannot be tuned away, a point we return to in \S\ref{sec:limitations}.
