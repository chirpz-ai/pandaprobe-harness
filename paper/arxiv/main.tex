...
rule written after turn $t$ can affect turn $t{+}1$; retired rules are excluded, duplicates are
removed, and the configured top-$k$ bounds prompt growth. The generated References index and four
read-only tools preserve on-demand access without routinely injecting the whole corpus. No mailbox
...
default, and any topic selected by managed repair. Each notice proposes a scope
(\texttt{global} for a trajectory finding, \texttt{scoped} for a step-level one) as a default the
repair agent may override. Scope is explicit
