...
that wraps an arbitrary LLM agent loop and, with no human in the loop, (i) \emph{detects} quality
degradations from PandaProbe's trace-level evaluation layer, (ii) posts structured diagnostic
notices to an inspectable workspace that the agent \emph{pulls} on its own initiative, (iii) lets
the agent author corrective operating \emph{rules} into a rule tree it owns, and (iv) admits each
rule only after an empirical \emph{evidence gate}: replay of the captured failure or a forward
...
regression; a per-turn barrier makes each admitted rule take effect \emph{within} the session that
...
$c(\Rset)$ & policy-inducing context: protocol $+$ index \\
$\idx$ & rule-\emph{index} operator (the References) \\
...
trace (\textbf{Detect}). The finding becomes a notice in the workspace (\textbf{Notice}); the agent
pulls it, inspects the flagged trace, reads the relevant rule file, and writes a rule into a scope
it owns (\textbf{Heal}); the candidate is promoted only after replay or a forward trial and is
retired if it regresses (\textbf{Validate}). A per-turn barrier closes the cycle before the next
turn, which is also what gives $\Gate$ a series to judge. The dashed line is the pull-model
boundary: the workspace is not injected into the conversation, and the system context carries only
the protocol and an \emph{index} of the rule files.}
\label{fig:overview}
\end{figure}
...
reaches PandaProbe through a CLI/JSON seam, keeping the core dependency-free and easy to exercise
against a stand-in. Failures degrade to pending scores and never raise into the host loop.
...
report has been handled, any case captured, and any notice posted, so the \emph{next} turn observes
the rule set and mailbox this turn produced. It runs on its own budget
(\texttt{barrier\_timeout\_s}, default 180\,s), separate from the best-effort drain joins used
elsewhere; on expiry the work continues detached and the turn is flagged as timed out, so a slow
evaluator degrades \emph{latency}, never correctness.
...
\subsection{The agent-owned rule workspace}
\label{sec:retrieval}
...
\Harness therefore separates the \emph{index} of what has been learned from the \emph{content}:
\begin{equation}
  c(\Rset)=c_0\oplus\idx(\Rset),
  \label{eq:context}
  % source: hook/context.py compose_system_preamble (skill root + banner; no rule bodies)
\end{equation}
where $c_0$ is a fixed root document (the standing protocol and tool table) and $\idx(\Rset)$ is a
generated directory of the rule \emph{files} that exist with their sizes, plus a compact banner when
notices are pending. Equation~\eqref{eq:context} contains \emph{no rule text}. Bodies enter context
only when the agent reads them, via a read-by-scope or search call it makes on its own initiative.
Hence $|c(\Rset)|$ grows with the number of rule \emph{files}, not rules, and the context cost of
adaptation is bounded independently of how much has been learned.

\paragraph{Scopes.}
The rule set is partitioned by a free-form \emph{scope} that is also the file a rule lives in:
\texttt{global} for whole-trajectory lessons that always apply, \texttt{scoped} as the step-level
default, and any topic file the agent creates. When several step-level rules share a topic the agent
may re-file them under a topic of its own naming and retire the originals; the new file appears in
$\idx(\Rset)$ automatically. Each notice proposes a scope (\texttt{global} for a trajectory finding,
\texttt{scoped} for a step-level one) as a \emph{default} the agent may override. Scope is explicit
rather than inferred, which corrects an earlier design that derived it from whether a rule happened
to carry retrieval tags and so silently made any rule authored outside a notice a permanent global.

\paragraph{Relevance ranking.}
Ranking moved to where the agent asks for it. With $Q$ the case-folded query token set, $T_r$ the
...
query is supplied ($k{=}8$, ties by score then recency then identifier). Candidates are exempt and
always render in full, since a candidate under trial must be in force to be measurable. This score
no longer selects what enters the prompt, because nothing does.
...
target metric, tags, and a \emph{scope} (defaulting to the one the notice proposed). Deterministic
Harness code executes \emph{gate}. Live rules (active plus candidate) are deduplicated by normalized
text and capped at 50, at which point an addition fails with a message telling the agent to retire
something first, so compaction is agent-driven rather than a silent eviction. Candidates render in
their scope file under an explicitly provisional heading: a rule must be in force to be measurable,
but every reader can see it is unproven, and the protocol tells the agent to prefer a validated rule
on conflict.
...
\label{alg:heal}
\begin{algorithmic}[1]
\STATE compose $c(\Rset)$ \hfill{\footnotesize\itshape protocol $+$ index; no rule text}
...
\STATE \textbf{barrier}: wait until the above has landed
\IF{agent pulls notice}
  \STATE inspect flagged trace; read suggested scope; author candidate $r$
...
diagnostics as untrusted data. Because $c(\Rset)$ carries no rule bodies and eval-derived text
reaches the agent only through explicit tool calls, the blast radius of a poisoned rule is a file the
agent chose to read rather than every prompt.

\emph{Ten} structured tools cover mailbox list, read, and acknowledge; trace inspection; and rule
read, list, search, add, retire, and status. Every tool returns an \texttt{ok} envelope and folds
errors into data. Size is a design parameter, not an accident: each additional capability is a turn the agent
might spend on harness bookkeeping instead of the task, so \Harness withholds surfaces that are
redundant with what a notice already carries (the score trajectory, which the notice reports) or
that invite the agent to audit its own process (a journal digest, a reflection summary). Corpus
curation is an operator concern, reached from code rather than from the agent.
...
index but no rule text; rule bodies live in the \texttt{rules/} tree the agent owns:
\begin{lstlisting}[language={}]
HARNESS_ROOT/
  harness_rules.md   <- skill root (no rule text)
...
# protocol + rule-file index + banner; no rule bodies
prompt = harness.system_context() + prompt

# settle=True awaits this turn's diagnosis, so a rule
# learned now is in force for the next turn
...
10 tools, the agent-owned rule tree, capture, replay, and candidate validation.
...
preamble and 10 tool schemas consume tokens every turn, so token, latency, and cost overhead, plus
barrier wait, are reported alongside success.
...
No-validation isolates the evidence gate. \emph{No-barrier} isolates in-session healing, and its
prediction is sharper than the others: without it a session contributes too few samples per series
for the stall window to close, so notices and promotions should fall toward zero and arm B should
collapse onto baseline-plus-overhead. \emph{Inlined rules} renders the whole corpus into the prompt
instead of the index of Eq.~\eqref{eq:context}, isolating pull-on-demand; we report the fraction of
agent tool calls that are harness operations beside $\passk$, since that ratio is the mechanism this
design protects. \emph{Tier-3 on} buys richer diagnoses at a cost scaling with metric count.
...
The preamble and ten tool schemas consume context every turn even when no rule is read, and the
barrier adds per-turn latency. Any $\passk$ change must be read next to
...
regression guard for prior wins. Equally important is what invokes the gate. \Harness scores every
...
available. Because the policy-inducing context carries an \emph{index} of what has been learned
rather than its contents, the cost of adaptation is decoupled from the size of what the agent has
learned. The implementation makes that loop practical as an inspectable, dependency-free envelope
around existing agent code. The empirical verdict is forthcoming.
