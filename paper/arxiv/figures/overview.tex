% Figure 1: PandaProbe Harness architecture and managed-repair loop.
...
\node[stage, below=14mm of detect]             (heal)    {\textbf{Managed repair}\\[1pt]\scriptsize package agent\\[-1pt]\scriptsize proposes rule};

% cyclic arrows Detect -> Notice -> Heal -> Validate -> Detect
\draw[->, thick] (detect) -- (notice);
\draw[->, thick] (notice) -- (heal);
\draw[->, thick] (heal) -- (validate);
...
% task agent receives only read-only guidance from the workspace
\draw[->, dashed] (ws.west) to[out=180,in=270] node[lbl,below left]{bounded guidance\\+ read-only rules} (agent.south);
